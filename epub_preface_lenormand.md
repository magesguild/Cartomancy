...
A reader trained in the English system will recognize the playing card identities on the Lenormand cards and find, at several points, direct conflicts between the oracle image and the cards English cartomantic meaning. Those conflicts are documented in the chapter that follows. They are not errors — they are the record of two independent traditions that developed in parallel from the same physical object without reference to each other.
